...
def save_aligned_image(image_path, args):
    ...
    # save data
        ...
        save_path_2d_lmks = (
            os.path.splitext(os.path.join(save_dir, "insight_lmks", os.path.basename(image_path)))[
                0
            ]
            + ".npy"
        )
        ...
        save_path_flame_params = (
            os.path.splitext(os.path.join(save_dir, "tmp_dicts", os.path.basename(image_path)))[0]
            + ".npy"
        )
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(video_paths))):
        ...
        parmap.map(
            save_aligned_image, img_paths, args, pm_pbar=True, pm_processes=int(num_cores * 0.55)
        )
