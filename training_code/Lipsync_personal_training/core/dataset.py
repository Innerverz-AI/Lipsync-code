    ...
    def __getitem__(self, index):
        ...
        while not FLAG:
            try:
                ...
                if video_url_name in self.forbidden_list or reg_video_url_name in self.forbidden_list or file_name+5 > self.frame_num_per_video or reg_file_name+5 > self.frame_num_per_video:
                    continue
                
                # data path list with time step T
                video_frames_path_list = [path for path in self.gt_img_path_list if (video_url_name in path and video_name in path)]
                ...
                reg_hubert_path = os.path.join(os.path.dirname(reg_gt_img_path.replace('aligned_imgs', 'mel')), '000000.npy')
                ...
                for i in range(T):
                    ...
                    reg_param_paths.append(os.path.splitext(reg_gt_img_path.replace('aligned_imgs', 'flame_params').replace('%06d'%reg_file_name, '%06d'%(reg_file_name+i)))[0] + '.npy')
                    ref_img_paths.append(ref_img_path.replace("%06d" % ref_file_name, "%06d" % (ref_file_name + i)))
                
                # init data list
                flame_params = {}
                reg_flame_params = {}
                reg_crossid_params = {}
                gt_imgs, guide_imgs, ref_imgs, hubert_features, reg_hubert_features, mel_features, reg_mel_features = [], [], [], [], [], [], []
                ...
                reg_hubert_all = torch.tensor(np.load(reg_hubert_path, allow_pickle=True).transpose(1,0)).float()
                mel_features.append(hubert_all[:,int(file_name/25*80):int(file_name/25*80)+16].unsqueeze(0))
                reg_mel_features.append(reg_hubert_all[:,int(reg_file_name/25*80):int(reg_file_name/25*80)+16].unsqueeze(0))
                if file_name == 0 or mel_features[0].shape[2] != 16 or reg_mel_features[0].shape[2] != 16:
                    continue
                ...
                for i in range(T):
                    ...
                    reg_hubert_features.append(reg_hubert_all[:,(reg_start_id-1+i):(reg_start_id-1+i)+16].unsqueeze(0)) # melmel
                    ...
                    for key in flame_params.keys():
                        ...
                        reg_flame_params[key].append(torch.from_numpy(reg_tmp_params[key]).unsqueeze(0))
                ...
                for key in flame_params.keys():
                    ...
                    reg_crossid_params[key] = torch.cat(reg_crossid_params[key], dim=0) # T, param_dim
                ...
                continue
        return [gt_imgs, guide_imgs, ref_imgs, flame_params, reg_crossid_params, hubert_features, reg_hubert_features, mel_features, reg_mel_features]
...
def divide_datasets(model, CONFIG):
    ...
    opt_img_path_list = sorted(utils.get_all_images(CONFIG['DATASET']['TRAIN_PATH']['OPT_IMG'], CONFIG['BASE']['FRAME_NUM_PER_VIDEO']))
    ...
    train_opt_img_path_list, valid_opt_img_path_list = utils.split_dataset(opt_img_path_list, CONFIG['BASE']['FRAME_NUM_PER_VIDEO'], CONFIG['BASE']['VAL_SIZE'])
