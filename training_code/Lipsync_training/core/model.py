...
class MyModel(ModelInterface):
    ...
    def go_step(self):
        self.batch_data_names = ['gt_img', 'guide_img', 'ref_img', 'flame_params', 'lipsync_feature', 'syncnet_feature']
        self.saving_data_names = ['gt_img', 'input', 'ref_img', 'output', 'result_img', 'crop_img']
    ...
    def run_G(self, run_dict):
        with torch.no_grad():
            ...
            run_dict['lipsync_feature'] = torch.cat([run_dict['lipsync_feature'][:, i] for i in range(run_dict['lipsync_feature'].shape[1])], dim=0).unsqueeze(1)
            
            # make masked face
            vis_dict = self.DC.decode(run_dict['flame_params'], original_image=run_dict['gt_img'], tform_invs=run_dict['flame_params']['tform_inv'])
            run_dict['mask'] = get_convexhull_mask(run_dict['gt_img'], vis_dict['landmarks2d_points']/2) # mask = polygon2mask((H,W), np.array([x1,y1],[x2,y2],[x3,y3]))
            run_dict['masked_face'] = run_dict['gt_img']*(1-run_dict['mask'])
            run_dict['input'] = run_dict['guide_img']*run_dict['mask'] + run_dict['masked_face']

        # run Lipsync model
        run_dict['output'] = self.G(run_dict['input'], run_dict['ref_img'], run_dict['lipsync_feature'])
        ...
        run_dict['mel_embedding'], run_dict['img_embedding'] = self.S(run_dict['syncnet_feature'], sync_img)
