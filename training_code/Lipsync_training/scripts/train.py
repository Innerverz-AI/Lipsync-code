    ...
    while CONFIG['BASE']['GLOBAL_STEP'] < CONFIG['BASE']['MAX_STEP']:
        ...
        if model.accelerator.is_main_process:
            ...
            if CONFIG['BASE']['GLOBAL_STEP'] % CONFIG['CYCLE']['TRAIN_IMAGE'] == 0:
                ...
                cv2.imwrite(f"{CONFIG['BASE']['SAVE_ROOT_IMGS']}/_latest_train_result.png", train_images_grid)
                cv2.imwrite(f"{CONFIG['BASE']['SAVE_ROOT_IMGS']}/{str(CONFIG['BASE']['GLOBAL_STEP']).zfill(8)}_train.png", train_images_grid)
