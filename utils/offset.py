...
class Sync_tool:
    ...
    def get_bbox(self):
        ...
        for idx, fpath in enumerate(frame_path_list):
            ...
            for bbox in bboxes:
                frame_dets.append(
                    {"index": idx, "frame": frame, "bbox": (bbox[:-1]).tolist(), "conf": bbox[-1]}
                )
    ...
    def get_feats(self, track_dets, mfcc):
        ...
        im_ts = torch.tensor(
            np.transpose(im_np, (0, 3, 4, 1, 2)), dtype=torch.float32
        )  # shape = (b, c, t, 224, 224)
        ...
        # 1sec -> 100
        minframe = min(
            math.floor(len(track_dets)), math.floor(mfcc.shape[-1] / 100 * self.opts.frame_rate)
        )
